...
<table class="table table-data2">
    ...
    <tbody>
        ...
        <tr class="tr-shadow">
            <td>{{$book->id}}</td>
            <td><b>{{$book->name}}</b></td>
            <td><b class="text-danger">{{$book->amount}}</b>/sp</td>
            <td><b class="text-warning">{{$book->price}}</b>-$</td>
            <td>
                @if ($book->level == 0)
                <b class="text-white bg-success">Giấy ăn</b>
            @endif
            @if ($book->level == 1)
                <b class="text-white bg-danger">Giấy vệ sinh</b>
            @endif
            <!-- @if ($book->level == 2)
                <b class="text-white bg-primary">Hải sản</b>
            @endif
            @if ($book->level == 3)
                <b class="text-white bg-secondary">Đông lạnh</b>
            @endif
            @if ($book->level == 4)
